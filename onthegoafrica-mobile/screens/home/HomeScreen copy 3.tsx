...
function HomeScreen({ navigation }: any) {
  ...
  const onSearchSubmit = async () => {
    ...
    const response = await handleSearch({ search: searchQuery });
  ...
  const onQuickPress = async (q: Quick) => {
    ...
    const response = await handleSearch({ search: q.id });
    setSearchResults(response);
  };
    ...
    <TouchableOpacity activeOpacity={0.9} style={s.card} onPress={onPress}>
      ...
      >
        ...
        <View style={s.cardInfo}>
          <Text style={s.cardMeta}>{v.distance || 0}</Text>
          <Text style={s.cardTitle}>{v.userName}</Text>
