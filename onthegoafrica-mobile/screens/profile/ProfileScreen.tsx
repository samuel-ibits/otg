...
import { getSavedProfile, logoutUser } from "../../api/api";
...
export default function ProfileScreen() {
  ...
  const [tab, setTab] = useState<
    "Posts" | "Reviews" | "Skills & Hobbies" | "Bookmarks"
  >("Posts");
  const [profile, setProfile] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const tabs = useMemo(
    () => ["Posts", "Reviews", "Skills & Hobbies", "Bookmarks"] as const,
    []
  );
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      try {
        const storedProfile = await getSavedProfile();
        ...
        if (storedProfile) {
          ...
          // navigation.navigate("OnboardingFlow", { screen: "CreateProfile" });
        }
      } catch (error) {
    ...
    return (
      <SafeAreaView style={s.root}>
        <View style={s.loaderContainer}>
          <ActivityIndicator size="large" color="#0a0f1cff" />
  ...
  const renderSkillsAndHobbies = () => (
    <View style={{ paddingHorizontal: 16, gap: 16, marginTop: 6 }}>
      <SkillsBlock items={profile.skills} onEdit={() => {}} />
      <HobbiesBlock items={profile.interests} onEdit={() => {}} />
      <PlacesToVisitBlock items={profile.placesVisited} onEdit={() => {}} />
    </View>
  );

  const listData =
    tab === "Posts" ? POSTS : tab === "Bookmarks" ? BOOKMARKS : [];
          ...
          ListEmptyComponent={
            tab === "Reviews" ? (
              <View style={{ padding: 24 }}>
                <Text style={{ color: "#8EA0BB", textAlign: "center" }}>
                  No reviews yet.
                </Text>
              </View>
            ) : null
...
function ProfileHeader({ profile }: { profile: any }) {
  ...
  const bio = profile?.bio || '"Update you Bio".';
  const businessType = profile?.profession || "Digital Marketer";
  const address = profile?.address || "Lagos, Nigeria";
  ...
  return (
    <View style={s.profWrap}>
      <Image source={profilePicture} style={s.profCover} />
      <View style={{ marginTop: 12 }}>
        <Text style={s.fullName}>{userName}</Text>
        <View style={s.followRow}>
          <Text style={s.followNum}>{following}</Text>
          <Text style={s.followLbl}> Following</Text>
          <Text style={s.followNum}> {followers}</Text>
        ...
        <InfoRow icon="💼" text={displayBusinessType} />
        <InfoRow icon="📍" text={address} />
        <InfoRow icon="⏰" text={`Joined ${joinDate}`} />
...
function RecentVisits({ profile }: { profile: any }) {
  return (
    <View style={{ paddingTop: 6, paddingBottom: 10 }}>
      ...
      <FlatList
        data={profile.placesVisited || VISITS}
        keyExtractor={(x) => x.id}
...
const s = StyleSheet.create({
  ...
  screenTitle: { color: "#0F1929", fontFamily: "RCB-Bold", fontSize: 20 },
